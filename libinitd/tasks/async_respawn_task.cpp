...
void async_respawn_task::add_child_listener(pid_t pid) {
    children.emplace(pid, make_unique<sysapi::wait_child>(pid, [this](sysapi::child_status& status){
                        process_sigtrap(status);
                     }));
}
...
void async_respawn_task::process_sigtrap(sysapi::child_status &status) {
    if (WIFSTOPPED(status.status)) {
        if (WSTOPSIG(status.status) == SIGTRAP) {
            if ((status.status >> 8) & (PTRACE_EVENT_FORK << 8)) {
                unsigned long grandchild_pid;
                ::ptrace(PTRACE_GETEVENTMSG, status.pid, 0, &grandchild_pid);
        ...
        ptrace(PTRACE_CONT, status.pid, 0, 0);
    } else if (WIFEXITED(status.status)) {
        remove_child_listener(status.pid);
    }
}
